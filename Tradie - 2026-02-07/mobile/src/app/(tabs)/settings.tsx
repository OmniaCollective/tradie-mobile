...
export default function SettingsScreen() {
  ...
  // Payment setup state
  const [paymentStatus, setPaymentStatus] = useState<'not_connected' | 'pending' | 'complete'>('not_connected');
  ...
  const handleCalendarToggle = async (value: boolean) => {
    if (value) {
      ...
      if (granted) {
        ...
        if (synced > 0) {
          Alert.alert('Calendar Synced', `${synced} job(s) added to your calendar.`);
        }
      } else {
        Alert.alert(
          'Permission Required',
          'Please enable calendar access in Settings to sync jobs.'
        );
      }
    } else {
  ...
  const handleDailyRemindersToggle = async (value: boolean) => {
    ...
    if (value) {
      ...
      Alert.alert(
        ...
        'You\'ll get a reminder at 6 PM to send customer notifications for tomorrow\'s jobs.'
      );
    }
  };
    ...
    try {
      ...
      if (result.success) {
        if (result.status === 'complete') {
          setPaymentStatus('complete');
          await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
          Alert.alert('Already Set Up', 'Your payment account is ready to receive payments!');
        ...
        <View className="px-4 pb-8">
          ...
          <Animated.View entering={FadeInDown.delay(600).duration(400)}>
            <View className="items-center py-6">
              ...
              <Text className="text-slate-500 text-sm mt-1">v1.0.0</Text>
            </View>
          </Animated.View>
        </View>
      </ScrollView>
