...
import { useTradeStore, useJobs, useCustomers, useTodos, Job, JobStatus } from '@/lib/store';
...
export default function DashboardScreen() {
  ...
  const [upgradeFeature, setUpgradeFeature] = useState<'bookingLinks' | 'customers'>('bookingLinks');

  const { isPro, canSendBookingLink, bookingLinksRemaining, bookingLinksSentThisMonth } = useProAccess();
  ...
  return (
    <ScrollView className="flex-1 bg-[#0F172A]">
          ...
          <Pressable
            onPress={() => {
              if (!canSendBookingLink) {
                setUpgradeFeature('bookingLinks');
                setShowUpgradePrompt(true);
              } else {
                router.push('/send-link');
              }
            }}
            className="bg-[#14B8A6] rounded-2xl p-4 mb-2 flex-row items-center justify-center active:opacity-80"
          >
            <Send size={20} color="#FFF" />
            <Text className="text-white font-bold text-base ml-2">Send Booking Link</Text>
          </Pressable>
          {/* Usage indicator */}
          {!isPro && (
            <View className="flex-row items-center justify-center mb-4">
              <Text className="text-slate-500 text-xs">
                {bookingLinksRemaining > 0
                  ? `${bookingLinksRemaining} of 15 free links remaining`
                  : 'No free links remaining'}
              </Text>
              {bookingLinksRemaining <= 5 && (
                <Pressable
                  onPress={() => router.push('/paywall')}
                  className="ml-2 flex-row items-center"
                >
                  <Crown size={12} color="#14B8A6" />
                  <Text className="text-[#14B8A6] text-xs ml-1">Upgrade</Text>
                </Pressable>
              )}
            </View>
          )}
          {isPro && <View className="mb-4" />}
        </Animated.View>
      ...
      <UpgradePrompt
        ...
        currentUsage={upgradeFeature === 'bookingLinks' ? bookingLinksSentThisMonth : customers.length}
      />
    </ScrollView>
  );
}
