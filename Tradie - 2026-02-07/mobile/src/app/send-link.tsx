...
import { X, Send, MessageCircle, Mail, Copy, Check, Crown } from 'lucide-react-native';
...
import { useProAccess, FREE_TIER_LIMITS } from '@/lib/useProAccess';
...
export default function SendLinkScreen() {
  ...
  // Generate a unique booking link (in production, this would come from your backend)
  const bookingId = Math.random().toString(36).substring(2, 8);
  const bookingLink = `https://tradie.app/book/${bookingId}`;
  ...
  const handleSendSMS = async () => {
    ...
    if (!isAvailable) {
      Alert.alert('SMS Not Available', 'SMS is not available on this device. Try email instead.');
      setSending(false);
      return;
    }

    const message = `Hi ${customerName},\n\nYou can book an appointment and get an instant quote using this link:\n\n${bookingLink}\n\nThanks,\n${businessName}`;

    try {
      const { result } = await SMS.sendSMSAsync([contactValue], message);
  ...
  const handleSendEmail = async () => {
    ...
    if (!isAvailable) {
      Alert.alert('Email Not Available', 'Email is not available on this device. Try SMS instead.');
      setSending(false);
      return;
    }

    const subject = `Book Your Appointment with ${businessName}`;
    const body = `
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 500px; margin: 0 auto;">
  <h2 style="color: #0F172A;">Hi ${customerName},</h2>
  <p style="color: #334155; font-size: 16px; line-height: 1.6;">
    You can book an appointment and get an instant quote using the link below:
  </p>
  <a href="${bookingLink}" style="display: inline-block; background: #14B8A6; color: white; padding: 14px 28px; border-radius: 12px; text-decoration: none; font-weight: bold; margin: 20px 0;">
    Book Now
  </a>
  <p style="color: #64748B; font-size: 14px; margin-top: 24px;">
    Or copy this link: <a href="${bookingLink}" style="color: #14B8A6;">${bookingLink}</a>
  </p>
    ...
    return (
      <View className="flex-1 bg-[#0F172A] items-center justify-center">
        ...
        >
          ...
          <Text className="text-white font-bold text-xl">Link Sent!</Text>
          <Text className="text-slate-400 mt-2">Customer will receive the booking link</Text>
        </Animated.View>
      </View>
    );
  }
    ...
    >
      <View className="flex-1 px-4 pt-4">
        ...
        <View className="flex-row items-center justify-between mb-6">
          ...
          <Text className="text-white font-bold text-lg">Send Booking Link</Text>
          <View className="w-10" />
        </View>
        ...
        <Animated.View entering={FadeInDown.delay(100).duration(400)}>
          <Text className="text-slate-400 text-sm mb-6">
            Send your customer a link to book an appointment and get an instant quote.
          </Text>
          ...
          <View className="bg-[#1E293B] rounded-xl p-4 mb-6 border border-[#334155]">
            <Text className="text-slate-400 text-xs mb-2">Booking Link</Text>
            <View className="flex-row items-center justify-between">
              <Text className="text-[#14B8A6] text-sm flex-1" numberOfLines={1}>
                {bookingLink}
              </Text>
              <Pressable
                onPress={handleCopyLink}
                className="ml-2 p-2 bg-[#0F172A] rounded-lg active:opacity-70"
              >
                {copied ? (
        ...
        <View className="mt-auto pb-8">
            ...
            <View className="flex-row items-center justify-center mb-3">
              <Text className="text-slate-500 text-sm">
                {bookingLinksRemaining > 0
                  ? `${bookingLinksRemaining} free link${bookingLinksRemaining !== 1 ? 's' : ''} remaining this month`
                  : 'No free links remaining'}
              </Text>
          ...
          >
            ...
            >
              {sending ? 'Sending...' : contactMethod && contactValue.trim() ? `Send via ${contactMethod === 'sms' ? 'SMS' : 'Email'}` : 'Send Booking Link'}
            </Text>
          </Pressable>
        </View>
      </View>
